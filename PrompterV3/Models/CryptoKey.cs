...
namespace PrompterV3.Models {
  ...
  public class CryptoKey:ICryptoKey {
    ...
    public async Task<string> ToCipherStringAsync(string messageToEncode) {
      ...
      return Convert.ToBase64String(mb);
    }
    ...
    public async Task<string> ToCipherBitsAsync(byte[] messageToEncode) {
      if((messageToEncode==null)||(messageToEncode.Length==0)) throw new ArgumentNullException(nameof(messageToEncode));
      if(_key is null) throw new CryptoKeyNotSetException();
      if(_iv is null) throw new CryptoKeyNotSetException();
      using(Aes aes = Aes.Create()) {
        aes.Key=_key;
        ...
        CryptoStream encodedStream = new CryptoStream(memoryStream, aes.CreateEncryptor(), CryptoStreamMode.Write);
        StreamWriter writer = new StreamWriter(encodedStream);
        var base64Encoded = Convert.ToBase64String(messageToEncode);
        await writer.WriteAsync(base64Encoded);
        writer.Close();
        encodedStream.Close();
    ...
    public async Task<byte[]> AsDecipherBitsAsync(string encodedMessage) {
      ...
      using(Aes aes = Aes.Create()) {
        aes.Key=_key;
        aes.IV=_iv;
        var debase64decode = Convert.FromBase64String(encodedMessage);
